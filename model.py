...
class CoupledModel:
    ...
    tendency:
        Deterministic drift/tendency function ``G(x)`` returning one tendency
        per state.
    diffusion:
        Optional stochastic forcing function ``g(x)``. It may return shape
        ``(n_states,)`` for one shared Wiener process or
        ``(n_states, n_forcings)`` for multiple independent Wiener processes.
    processes:
        Optional named deterministic process tendencies. Their sum should
        reproduce ``tendency`` when a complete process budget is supplied.
    units:
        Optional state-unit labels.
    time_unit:
        Model time unit, e.g. ``"day"``.
    """
