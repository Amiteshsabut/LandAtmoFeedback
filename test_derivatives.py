...
from landfeedback import derivative_convergence, diffusion_jacobian, jacobian
...
def test_derivative_convergence_table():
    point = np.array([2.0, 3.0])
    expected = np.array([[4.0, 1.0], [3.0, 2.0]])
    frame = derivative_convergence(
        nonlinear_function, point, reference=expected, methods=("central", "complex")
    )
    assert set(frame.columns) == {"method", "rel_step", "max_abs_error", "rms_error"}
    assert frame["max_abs_error"].min() < 1e-10
